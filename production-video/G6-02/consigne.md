...
Objectif unique : faire retenir que un scénario d'avenir est une proposition qu'on peut démonter — s'il ne se démonte pas, ce n'est plus un scénario.
...
5. l'outil « La maquette », énoncé exactement comme dans la source ;
...
Emplacement `medias.transformation` du module G6-02 — donc **après le verdict**.
...
renseigner à la main dans `site/modules/G6-02.json`. NotebookLM fournit le
